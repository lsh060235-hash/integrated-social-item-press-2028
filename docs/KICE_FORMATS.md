...
한글 실제 PDF에서 머리말 가로선·본문 전체 높이 세로선을 확인한다. python-hwpx의 머리말 사본 동기화와 선 좌표 정규화가 필요했다. v0.2는 마지막 쪽의 두 문항이 모두 좌단에 있을 때 마지막 문항을 우단으로 옮기고 다시 검증한다. 특정 회차 번호로 배치를 강제하지 않는다. 6쪽 강제 축소보다 원문 보존·가독성을 우선한다.

v0.2 보완: 짧은 선택지의 가로 배열, 원문에 명시된 두 지역 응답 제목, 긴 표 제목의 단위 줄 구분, 문항 사이 간격 축소. 실제 렌더에서 문항이 단을 넘으면 검증 실패다. `item-review/index.html`은 문항별 PDF 위치·높이와 공식 예시 대조 이미지를 제공하고 검토 상태는 자동 승인하지 않는다. 기존 내부 8/10 점수는 공식 형식 준수 인증이 아니다.
